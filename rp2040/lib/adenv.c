...
void AdEnv_Init(AdEnv* env, float sample_rate)
{
    ...
    env->curve_scalar    = 0.0f; // full linear
    env->phase           = 0;
    env->min             = 0.0f;
    env->max             = 1.0f;
    env->output          = 0.0001f;
...
float AdEnv_Process(AdEnv* env)
{
    ...
    time_samps = (uint32_t)(env->segment_time[env->current_segment] * env->sample_rate);
    ...
    if(env->curve_scalar == 0.0f)
    {
        env->c_inc = (end - beg) / time_samps;
    }
    else
    {
        env->c_inc = (end - beg) / (1.0f - expf_fast(env->curve_scalar));
    }
    ...
    {
        env->curve_x += (env->curve_scalar / time_samps);
