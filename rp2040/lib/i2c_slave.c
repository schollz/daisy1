...
void i2c_slave_init(i2c_inst_t *i2c, uint8_t address,
  ...
  // Note: The I2C slave does clock stretching implicitly after a RD_REQ, while
  // the Tx FIFO is empty. There is also an option to enable clock stretching
  // while the Rx FIFO is full, but we leave it disabled since the Rx FIFO
  // should never fill up (unless slave->handler() is way too slow).
  ...
  irq_set_exclusive_handler(
      num, i2c_index == 0 ? i2c0_slave_irq_handler : i2c1_slave_irq_handler);
